...
def make_loan_interest_accrual_entry(
	...
	if flt(interest_amount, precision) > 0:
		...
		# During a bulk repost nobody is watching the UI, so skip the per-doc
		# realtime publish (doc_update/list_update) which is pure overhead there.
...
def calculate_penal_interest_for_loans(
	...
	# Batch-fetch principal outstanding for all demands instead of one query per demand.
	# The (loan, repayment_schedule_detail, EMI, Principal) filter is NOT unique:
	# cancelled + submitted duplicates coexist (common right after a repost). The
	# original per-demand get_value returned the row ordered by "creation DESC"
	# (Loan Demand sort_order is DESC), limit 1 -- i.e. the newest matching row.
	# Mirror that exactly: order by creation DESC and keep the FIRST row seen per
	...
	for demand in demands:
		...
		if getdate(posting_date) >= add_days(getdate(demand.demand_date), grace_period_days):
			...
			# pending_amount is constant across the daterange loop; compute the flt once.
			pending_amount = flt(demand.pending_amount)

			# get_per_day_interest only varies by the calendar year of current_date
			# (and only under Actual/Actual). Cache by year to avoid recomputing daily.
