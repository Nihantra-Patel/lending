...
class TestLoan(IntegrationTestCase):
	...
	def test_interest_accrual_overlap(self):
		for frequency in ["Monthly", "Daily"]:
			...
			make_loan_disbursement_entry(
				loan.name, loan.loan_amount, disbursement_date="2024-12-30", repayment_start_date="2025-01-05"
			)

			# Process Loan Interest Accrual
			process_loan_interest_accrual_for_loans(
				posting_date="2025-01-03", loan=loan.name, company="_Test Company"
			)
			process_loan_interest_accrual_for_loans(
			...
			repayment = create_repayment_entry(loan.name, "2025-01-05", 1150, repayment_type="Pre Payment")

			repayment.submit()
			process_loan_interest_accrual_for_loans(
				posting_date="2025-01-06", loan=loan.name, company="_Test Company"
			)
